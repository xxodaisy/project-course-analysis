-- detection of duplicate data from user_id
select user_id, count(*)
from users u
group by u.user_id 
having count(*) > 1;

-- detection of duplicate data from course_id
select course_id, count(*)
from courses c 
group by c.course_id
having count(*) > 1;

--detection of duplicate data from voucher_id
select voucher_id, count(*)
from vouchers v 
group by v.voucher_id  
having count(*) > 1;

-- detection of duplicate data from order_id
select order_id, count(*)
from orders o 
group by o.order_id
having count(*) > 1;

-- detection of duplicate from order_detail_id
select order_detail_id, count(*)
from order_details od 
group by od.order_detail_id
having count(*) > 1;

-- detection of duplicate from redemption_id
select redemption_id, count(*)
from redemption r
group by r.redemption_id
having count(*) > 1;

-- detection of duplicate from completion_id
select completion_id, count(*)
from completion comp 
group by comp.completion_id
having count(*) > 1;

--detection from outliers
select * 
from order_details od
where od.price <= 0;
