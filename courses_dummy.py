...
# def normalize_date(dt):
#     """
#     Convert date/string/Timestamp to datetime (jam 00:00 kalau ga ada jam)
#     """
#     if pd.isna(dt) or dt is None:
#         return None
#     if isinstance(dt, pd.Timestamp) or isinstance(dt, datetime):
#         return pd.to_datetime(dt)
#     return pd.to_datetime(datetime.combine(dt, time.min))
...
# vouchers_df.to_csv("vouchers.csv", index=False)
# orders_df.to_csv("orders.csv", index=False)
# order_details_df.to_csv("order_details.csv", index=False)
# redemption_df.to_csv("redemption.csv", index=False)
# completion_df.to_csv("completion.csv", index=False)

# # --- TRANSACTIONS ---
# transactions = []
# user_ids = [u["user_id"] for u in users]  # asumsi tabel users sudah ada
# for _ in range(100000):
#     user_id = random.choice(user_ids)
#     course = random.choice(courses)
#     enrollment_date = fake.date_time_between(start_date="-1y", end_date="now")
#     transaction_date = enrollment_date + timedelta(days=random.randint(1, 7))

#     #ambil voucher user kalau belum dipakai
#     user_voucher = next((v for v in vouchers if v["user_id"] == user_id and v["transaction_id"] is None), None)
#     selected_voucher = random.choice(vouchers) if random.random() < 0.6 else None
#     voucher_code = selected_voucher["voucher_code"] if selected_voucher else "No Voucher" 

#     discount_percent = 0
#     if selected_voucher:
#         start = ensure_datetime(selected_voucher["start_date"])
#         end = ensure_datetime(selected_voucher["end_date"])
#         if transaction_date <= start and transaction_date <= end:
#             discount_percent = selected_voucher["discount_percent"]

#     final_price = round(course["price"] * (1 - discount_percent / 100), 2)
...
#         "user_id": user_id,
#         "course_id": course["course_id"],
#         "voucher_id": selected_voucher["voucher_id"] if selected_voucher else None,
...
#     #lock voucher: tandai sudah dipakai
#     if selected_voucher:
#         selected_voucher["transaction_id"] = transaction_id

# transactions_df = pd.DataFrame(transactions)

# # --- fungsi generate dates ---
# def generate_dates(transaction_date, has_voucher=False, voucher_valid=True):
#     redeem_date = None
#     enrollment_date = None
#     completion_date = None

#     if has_voucher: #transaksi pake voucher
#         redeem_date = transaction_date + timedelta(days=random.randint(1,7))
#         if voucher_valid: #voucher valid -> bisa lanjut enroll dan complete
#             enrollment_date = redeem_date + timedelta(days=random.randint(1,7))
#             completion_date = enrollment_date + timedelta(days=random.randint(7,90))
#     else: #transaksi tanpa voucher
#         enrollment_date = transaction_date + timedelta(days=random.randint(1,7))
#         completion_date = enrollment_date + timedelta(days=random.randint(7,90))

#     return redeem_date, enrollment_date, completion_date
...
#     voucher = next((v for v in vouchers if v["voucher_id"] == voucher_id), None) if voucher_id else None

#     #status voucher
#     if voucher_id and voucher:
#         start = ensure_datetime(voucher["start_date"])
#         end = ensure_datetime(voucher["end_date"])
#         redeem_temp = transaction_date + timedelta(days=random.randint(1,7))
#         if start <= redeem_temp <= end:
#             redemption_status = "Used"
#             voucher_valid = True 
...
#             voucher_code = None
#             voucher_used = False
#     elif voucher_id  and not voucher:
#         redemption_status = "Invalid"
#         voucher_valid = False
#         voucher_code = None
#         voucher_used = False
...
#     #panggil generate_dates untuk memastikan urutan tanggal
#     redeem_date, enrollment_date, completion_date = generate_dates(
#         transaction_date, has_voucher=bool(voucher_id), voucher_valid=(redemption_status == "Used")
#     )

#     # Simpan redemption
#     redemption.append({
#         "redemption_id": fake.uuid4(),
#         "transaction_id": transaction["transaction_id"],
#         "voucher_id": voucher_id,
#         "user_id": transaction["user_id"],
#         "course_id": transaction["course_id"],
#         "voucher_code": voucher_code if voucher_code else "No Voucher",
#         "voucher_used": voucher_used,
#         "redemption_date": redeem_date,
#         "redemption_status": redemption_status,
#         "enrollment_date": enrollment_date,
#         "transaction_date": transaction_date
#     })
...
#     completion.append({
#         "completion_id": fake.uuid4(),
#         "transaction_id": transaction["transaction_id"],
#         "user_id": transaction["user_id"],
#         "course_id": transaction["course_id"],
#         "completion_date": completion_date,
#         "completion_status": "Completed" if completion_date else "Not Completed"
#     })
